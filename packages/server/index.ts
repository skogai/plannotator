      ...
      server = Bun.serve({
        ...
        async fetch(req, server) {
          ...
          if (url.pathname === "/api/config" && req.method === "POST") {
            try {
              const body = (await req.json()) as { displayName?: string; diffOptions?: Record<string, unknown>; conventionalComments?: boolean; conventionalLabels?: unknown[] | null; pfmReminder?: boolean; sketchDiagrams?: boolean };
              ...
              if (body.sketchDiagrams !== undefined) toSave.sketchDiagrams = body.sketchDiagrams;
